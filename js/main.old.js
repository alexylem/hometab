document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('notesApp', () => ({
        ...
        async removeNote(id) {
            const collectionRef = this.getCollectionRef();
            if (!collectionRef) return;
            if (confirm('Supprimer cette note ?')) { try { await collectionRef.doc(id).delete(); } catch (e) { console.error("Error removing note:", e); } }
        }
    }));
    ...
    Alpine.data('todoApp', () => ({
        ...
        get allPeople() { const c = this.items.filter(t => !t.completed).flatMap(t => t.people || []).reduce((acc, n) => { acc[n] = (acc[n] || 0) + 1; return acc; }, {}); return Object.entries(c).map(([n, cnt]) => ({ name: n, count: cnt })).sort((a, b) => b.count - a.count); },
        ...
        get todayTodos() { return this.baseFilteredTodos.filter(t => !t.completed && t.isToday); },
        get laterTodos() { return this.baseFilteredTodos.filter(t => !t.completed && !t.isToday); },
        ...
        async addTodo() {
            ...
            try {
                await collectionRef.add({ text: txt, completed: false, isToday, people: [...new Set(mentions)], createdAt: firebase.firestore.FieldValue.serverTimestamp() });
                this.newTodo = '';
        ...
        async removeTodo(id) {
            const collectionRef = this.getCollectionRef();
            if (!collectionRef) return;
            if (confirm('Supprimer cette tâche ?')) try { await collectionRef.doc(id).delete(); } catch (e) { console.error(e); }
        },
        ...
        async toggleCompletion(todoId) {
            ...
            if (todo) await collectionRef.doc(todoId).update({ completed: !todo.completed });
        },
    }));
